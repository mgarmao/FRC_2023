...
import frc.robot.subsystems.ArmCam;

public class ReverseTrackCube extends CommandBase {
    ArmCam armCam = new ArmCam();
    ...
    public ReverseTrackCube(double distanceToMoveInches, double m_speedLimit) {
        speedLimit = m_speedLimit;
        DISTANCE_TO_MOVE = distanceToMoveInches;
        addRequirements(armCam);
    ...
    @Override
    public void execute(){
        SmartDashboard.putNumber("Cone Yaw",armCam.getCubeYaw());
        PID = pid.calculate(armCam.getCubeYaw(), 0);
